...
class ajax extends AWS_CONTROLLER
{
    ...
    public function modify_question_action()
    {
        ...
        if (!$_POST['question_detail']) {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请补充问题内容')));
        }else{
            $_POST['question_detail'] = filter_Emoji($_POST['question_detail']);
        }
        ...
        H::ajax_json_output(AWS_APP::RSM(array(
            'url' => get_js_url('/question/' . $question_info['question_id'] . '?rf=false')
        ), 1, null));
    ...
    public function publish_question_action()
    {
        ...
        if ($this->user_id && HTTP::get_cookie('taobihu_add_draft_'.$add_draft_type.'_'.$this->user_id))
        {   
            HTTP::set_cookie('taobihu_add_draft_'.$add_draft_type.'_'.$this->user_id,'',0);
        } 
    ...
    public function publish_article_action()
    {
        ...
        if (!$_POST['message']) {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请补充文章内容')));
        }else{
            $article_submessage = filter_Emoji($_POST['message']);

            $m1=array();
            $strbefore = htmlspecialchars_decode($article_submessage);
            ...
            if (cjk_strlen($strbefore) < 350) {
                H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('文章内容不得少于350个字')));
            }else{
              $_POST['message'] = $article_submessage;
            }
        }
    ...
    public function article_cover_upload_action($article_id,$column_name,$max_size)
    {
        ...
        if($image_info[2] == 6)
        {
            $update_data['article_img'] = $this->model('article')->get_cover($article_id, 'real', 3);

        }else
        {
            $update_data['article_img'] = $this->model('article')->get_cover($article_id, null, 1) . basename($thumb_file['min']);
        } 
