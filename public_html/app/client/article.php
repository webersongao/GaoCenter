...
class article extends AWS_CONTROLLER
{
    ...
    public function index_action()
    {
        ...
        if ($article_info['user_info']) {
            $article_info['user_info'] = $this->model('client')->get_clean_user_info($article_info['user_info']);
        }

        //点赞者信息
        if (!empty($article_info['vote_users'])) {
            foreach ($article_info['vote_users'] as $key => $value) {
                $article_info['vote_users'][$key] = $this->model('client')->get_clean_user_info($value);
            }
        }
        ...
        if (!empty($comments)) {
            foreach ($comments as $key => $value) {
                if (!empty($value['user_info'])) {
                    $comments[$key]['user_info'] = $this->model('client')->get_clean_user_info($value['user_info']);
                }

                if (!empty($value['at_user_info'])) {
                    $comments[$key]['at_user_info'] = $this->model('client')->get_clean_user_info($value['at_user_info']);
                }
            }
        }
