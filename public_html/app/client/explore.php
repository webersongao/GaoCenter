...
class explore extends AWS_CONTROLLER
{
    ...
    public function index_action()
    {
        ...
        $question_key  = array('post_type', 'question_id', 'question_content', 'add_time', 'answer_count', 'view_count',/* 'answer_users',*/ 'question_reply', 'topics', 'user_info');
        $article_key   = array('post_type', 'id', 'title', 'message', 'add_time', 'views', 'comments', 'topics', 'user_info','article_img');
        ...
        if ($posts_list) {
            foreach ($posts_list as $key => $val) {
                ...
                foreach ($val as $k => $v) {
                    if (!in_array($k, $posts_list_key)) {
                        unset($posts_list[$key][$k]);
                    }
                }
                ...
                if ($val['user_info']) {
                    foreach ($val['user_info'] as $k => $v) {
                        if (!in_array($k, $user_info_key)) {
                            unset($posts_list[$key]['user_info'][$k]);
                        }
                    }

                    if ($val['post_type'] == 'question') {
                        $user_answer_new =$this->model('answer')->get_last_one_answer_by_uid_and_question_id($posts_list[$key]['user_info']['uid'],$val['question_id']);
                        $posts_list[$key]['question_reply'] = is_null($user_answer_new) ? '' : $user_answer_new;
                    }
                    ...
                    foreach ($val['topics'] as $kk => $vv) {
                        foreach ($vv as $k => $v) {
                            if (!in_array($k, $topics_key)) {
                                unset($posts_list[$key]['topics'][$kk][$k]);
                            }
                ...
                if (is_array($val['answer_users'])) {
                    foreach ($val['answer_users'] as $kk => $vv) {
                        foreach ($vv as $k => $v) {
                            if (!in_array($k, $user_info_key)) {
                                unset($posts_list[$key]['answer_users'][$kk][$k]);
                            }
                ...
                $posts_list[$key]['answer_users'] = is_null(array_values($posts_list[$key]['answer_users'])) ? [] : array_values($posts_list[$key]['answer_users']);
        ...
        H::ajax_json_output(AWS_APP::RSM(array(
            'total_rows' => count($posts_list_new),
            'rows'       => $posts_list_new,
        ), 1, null));
    }
}
