...
class ajax extends AWS_CONTROLLER
{
	...
	public function get_user_info($username,$password){
		if(preg_match("/^1[34578]\d{9}$/", $username)){
			$user_info = $this->model('account')->check_mobile($username, $password);
		}else{
			$user_info = $this->model('account')->check_login($username, $password);
		}
		return $user_info;
	}
	    ...
	    public function save_add_draft_action()
    {
        ...
        if(HTTP::set_cookie('taobihu_add_draft_'.$_POST['type'].'_'.$this->user_id,$_POST['message'],time()+3600*24, "/"))
        {
            H::ajax_json_output(AWS_APP::RSM(null, 1, AWS_APP::lang()->_t('已保存草稿, %s', date('H:i:s', time()))));
        }
    ...
    public function delete_add_draft_action()
    {
        ...
        if(HTTP::set_cookie('taobihu_add_draft_'.$_POST['type'].'_'.$this->user_id,'',0))
        {
            H::ajax_json_output(AWS_APP::RSM(null, 1, null));
        }
