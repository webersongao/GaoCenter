...
class main extends AWS_CONTROLLER
{
	...
	public function index_square_action()
	{
		...
		{
			case 'focus':
				if ($topics_list = $this->model('topic')->get_focus_topic_list($this->user_id, calc_page_limit($_GET['page'], 24)))
				{
			...
			case 'hot':
				...
				{
					if ($topics_list = $this->model('topic')->get_topic_list(null, $order, 24, $_GET['page']))
					{
			...
			case 'topic':
				if (!$topics_list = AWS_APP::cache()->get('square_parent_topics_topic_list_' . intval($_GET['topic_id']) . '_' . intval($_GET['page'])))
				{
					...
					if ($topics_list = $this->model('topic')->get_topic_list('topic_id IN(' . implode(',', $topic_ids) . ') AND merged_id = 0', 'discuss_count DESC', 24, $_GET['page']))
					{
						$topics_list_total_rows = $this->model('topic')->found_rows();
		...
		if ($this->user_id) {
			if ($my_topics_list = $this->model('topic')->get_focus_topic_list($this->user_id, calc_page_limit($_GET['page'], 5)))
			{
			TPL::assign('my_topics_list', $my_topics_list);
			}
		}
