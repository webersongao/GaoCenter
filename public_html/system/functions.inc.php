...
function show_error($exception_message, $error_message = '')
{
	...
	{
		AWS_APP::mail()->send('error@yiwenbuzhi.com', '[' . G_VERSION . '][' . G_VERSION_BUILD . '][' . base_url() . ']' . $error_message, nl2br($exception_message), get_setting('site_name'), 'WeCenter');
	}
...
function is_mobile($ignore_cookie = false)
{
	...
	{
		return false;  // 强制返回true 即默认使用响应式布局
	}
