	...
	ajax_request: function(url, params)
	{
		...
		function _callback (result)
		{
			...
			else if (result.errno == 1)
			{
				...
				// console.log(parseInt($(".aw-question-comment").find('h2.hidden-xs').text()));
	...
	ajax_processer: function (type, result)
	{
		...
		else
		{
			if (type == 'comments_form')
			{
				...
				$('#comment_count'+result.rsm.item_id).html(parseInt($('#comment_count'+result.rsm.item_id).html())+1);
				console.log(result.rsm.item_id);
			}
