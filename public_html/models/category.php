...
{
	public function update_category_info($category_id, $title, $parent_id, $url_token,$category_type ='question')
	{
	...
	public function add_category($type, $title, $parent_id,$url_token)
	{
		$category_url_tolen=$url_token?$url_token:$parent_id;

		return $this->insert('category', array(
			'type' => $type,
			'title' => $title,
			'url_token' => $category_url_tolen,
			'parent_id' => intval($parent_id),
		));
	}
