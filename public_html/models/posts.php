		...
		else
		{
			...
			if (isset($answer_count))
			{
				...
				if ($answer_count == 0)
				{
					...
					$where[] = "answer_count = 0";// . $answer_count;
				}
		...
		foreach ($posts_index as $key => $data)
		{
			switch ($data['post_type'])
			{
				...
				case 'article':
					...
					if ($explore_list_data[$key]['article_img']) {
						$explore_list_data[$key]['article_img'] = get_setting('upload_url') . '/article/cover/'.$explore_list_data[$key]['article_img'];	
					}else{
						$explore_list_data[$key]['article_img'] = '/static/css/default/img/default-cover.jpg';
					}
