		...
		foreach ($log_list as $key => $log)
		{
			...
			switch ($log['associate_action'])
			{
				...
				case ACTION_LOG::ADD_TOPIC : //添加话题
					...
					$title_list = $user_name_string . AWS_APP::lang()->_t(' 给该问题添加了一个话题').' <p><a href="topic/' . $topic_info['url_token'] . '">' . $log['associate_content'] . '</a>';
				...
				case ACTION_LOG::DELETE_TOPIC : //移除话题
					...
					$topic_info = $this->model('topic')->get_topic_by_id($log['associate_attached']);
					$title_list = $user_name_string . AWS_APP::lang()->_t(' 移除了该问题的一个话题').' <p><a href="topic/' . $topic_info['url_token'] . '">' . $log['associate_content'] . '</a>';
