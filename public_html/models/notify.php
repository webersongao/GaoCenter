		...
		foreach ($notify_list as $key => $notify)
		{
			...
			switch ($notify['model_type'])
			{
				case self::CATEGORY_CONSULT:
					...
					$tmp_data['key_url'] = get_js_url('/consult/' . $data['consult_info']['id']);
			...
			else
			{
				switch ($val['action_type'])
				{
                    ...
                    case self::TYPE_MEMBER_SUCCESS:
						...
						break;
					case self::TYPE_ALLOT__MONEY:
						$data[$key]['message'] = AWS_APP::lang()->_t('你在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'.AWS_APP::lang()->_t('中的回答获得了赏金');
