...
{
	public function add_nav_menu($title, $description, $type = 'custom', $type_id = 0, $link = null,$catagory_type = '')
	{
	...
	public function get_nav_menu_list($app = null)
	{
		...
		// 	switch ($app) {
		// 		case 'question':
		// 			$nav_menu_data = $this->fetch_all('nav_menu', 'catagory_type = "question"','sort ASC');
		// 			break;
				
		// 		case 'article':
		// 			$nav_menu_data = $this->fetch_all('nav_menu', 'catagory_type = "article"','sort ASC');
		// 			break;

		// 		case 'project':
		// 			$nav_menu_data = $this->fetch_all('nav_menu', 'catagory_type = "project"','sort ASC');
		// 			break;
		// 				}
		...
		switch ($app) {
			case 'question':
			if (!$nav_menu_data = AWS_APP::cache()->get('nav_menu_list_question')) {
				$nav_menu_data = $this->fetch_all('nav_menu', 'catagory_type = "question"','sort ASC');
				AWS_APP::cache()->set('nav_menu_list_question', $nav_menu_data, get_setting('cache_level_low'), 'nav_menu');
			}
			break;
		
			case 'article':
			if (!$nav_menu_data = AWS_APP::cache()->get('nav_menu_list_article')) {
				$nav_menu_data = $this->fetch_all('nav_menu', 'catagory_type = "article"','sort ASC');
				AWS_APP::cache()->set('nav_menu_list_article', $nav_menu_data, get_setting('cache_level_low'), 'nav_menu');
			}
			break;

			case 'project':
			if (!$nav_menu_data = AWS_APP::cache()->get('nav_menu_list_project')) {
				$nav_menu_data = $this->fetch_all('nav_menu', 'catagory_type = "project"','sort ASC');
				AWS_APP::cache()->set('nav_menu_list_project', $nav_menu_data, get_setting('cache_level_low'), 'nav_menu');
			}
			break;
